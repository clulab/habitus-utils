...
sys.path.append(
    "/Users/prcohen/anaconda2/envs/aPRAM/Habitus/Data/Data Objects/Code and Notebooks"
)
...
class DO_Encoder:
    ...
    def make_search_terms(self, *attributes):
        ...
        parts = [
            item
            for item in [self.__dict__.get(field) for field in fields]
            if item is not None
        ]
...
class DO_Decoder(SimpleNamespace):
    def __init__(self, json_string):
        ...
        self.df = pd.read_json(
            self.__dict__.get("df"), orient="split", convert_axes=False
        )
    ...
    def describe(self, *attributes, display=True):
        ...
        if attributes is ():
            to_describe = [
                x for x in self.__dict__.keys() if x not in ["df", "search_terms"]
            ]
        else:
    ...
    def cols(self, *columns):
        ...
        if columns is ():
            ...
            if len(valid_cols) < len(columns):
                print(
                    f"At least one of the columns you requested isn't in {self.name} dataframe"
                )
...
class Decoded_CGAP_DOs(Decoded_DOs):
    ...
    def get_all_columns_given_country(
        self, QNS_TO_AVOID, COUNTRY, REGEX_QNS_TO_AVOID, SURVEY_QN_TO_PREDICT
    ):
        df_single = self.concat_all_single_answer_qns(
            QNS_TO_AVOID, COUNTRY, REGEX_QNS_TO_AVOID, SURVEY_QN_TO_PREDICT
        )
        df_multiple = self.concat_all_multiple_answer_qns(
            QNS_TO_AVOID, COUNTRY, REGEX_QNS_TO_AVOID, SURVEY_QN_TO_PREDICT
        )
        df_combined = pd.concat([df_single, df_multiple], axis=1)
        return df_combined

    def concat_all_multiple_answer_qns(
        self, qns_to_avoid, country, REGEX_QNS_TO_AVOID, SURVEY_QN_TO_PREDICT
    ):
        df = None
        for k, v in tqdm(
            ...
            if v is not None:
                if country in k:
                    if (
                        not "HH" in k
                    ):  # most of the qns starting with HH are housekeeping ones and hence usually strings
                        if not v.df.empty:
                            if v.qtype == "multi" or v.qtype == "multiple":
                                ...
                                if label not in qns_to_avoid:
                                    for regex in REGEX_QNS_TO_AVOID:
                                        if (regex == "") or not (
                                            re.match(regex, label)
                                        ):  # if regex is empty it means add everything
                                            # attach the column name as qn_subtpe. eg: A5_Rice
                                            new_cols = []
                                            for c in v.df.columns:
                                                new_col_name = label + "_" + c
                                            ...
                                            v.df.columns = new_cols
                                            for sub_qn in v.df:
                                                v_df_scaled = scale_min_max(
                                                    v.df[sub_qn]
                                                )
                                                v.df[sub_qn] = v_df_scaled.reshape(-1)
                                                df = pd.concat(
                                                    [df, v.df[sub_qn]], axis=1
                                                )
    ...
    def concat_all_single_answer_qns(
        self, qns_to_avoid, country, REGEX_QNS_TO_AVOID, SURVEY_QN_TO_PREDICT
    ):
        df = None
        for k, v in tqdm(
