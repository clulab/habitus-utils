...
sys.path.append(
    "/Users/prcohen/anaconda2/envs/aPRAM/Habitus/Data/CGAP/Data/Data Objects/"
)
...
def get_data():
    ...
    for country in countries:
        for survey in ["sr", "rr"]:
            data_dict[country + "_" + survey] = read_csv(
                country + "_" + survey + "_clean.csv"
            )
    ...
    for country in countries:
        for survey in ["sr", "rr", "hh"]:
            data_dict[country + "_" + survey].set_index(
                "HHID", drop=False, inplace=True
            )
...
for var in ["A6", "A8", "A26", "A37"]:
    data_dict["bgd_rr"][var].replace(
        {v: master_crop_dict[k] for k, v in bgd_crops.items()}, inplace=True
    )
    data_dict["cdi_rr"][var].replace(
        {v: master_crop_dict[k] for k, v in cdi_crops.items()}, inplace=True
    )
    data_dict["nga_rr"][var].replace(
        {v: master_crop_dict[k] for k, v in nga_crops.items()}, inplace=True
    )
    data_dict["tan_rr"][var].replace(
        {v: master_crop_dict[k] for k, v in tan_crops.items()}, inplace=True
    )
...
for label, text in zip(labels, texts):
    Q = [
        Question_Encoder(
            ...
            label=label,
            answers={
                k: master_crop_dict.get(k) for k in eval(country + "_crops").keys()
            },
            country=country,
            survey="rr",
        )
...
for c in countries:
    data_dict[c + "_rr"]["A13"].replace(
        {v: master_livestock_dict[k] for k, v in eval(c + "_livestock").items()},
        inplace=True,
    )
...
Q = [
    Question_Encoder(
        ...
        answers={
            "would_use_more": 4,
            "sufficient": 3,
            "intermittent_but_ok": 2,
            "intermittent_not_ok": 1,
        },
        country=country,
        survey="rr",
    )
...
Q = [
    Question_Encoder(
        ...
        column_dict={
            ...
            "damage": 3,
            "lack_storage": 4,
            "lack_refrigeration": 5,
            "unreliable_middlemen": 6,
            "no_challenges": 7,
            "other": 8,
        },
        country=country,
...
Q[2].column_dict = {
    ...
    "damage": 5,
    "lack_storage": 6,
    "lack_refrigeration": 7,
    "unreliable_middlemen": 8,
    "no_challenges": 9,
    "other": 10,
}
...
Q = [
    Question_Encoder(
        ...
        column_dict={
            ...
            "want_ag_work_only": 2,
            "want_expand": 3,
            "would_take_offered_job": 4,
            "am_satisfied": 5,
            "my_legacy": 6,
            "make_ends_meet": 7,
            "want_children_continue": 8,
        },
        country=country,
        survey="sr",
    )
...
Q[0].column_dict.update(
    {"ICM_CLUB": 9, "water_users": 10, "livestock": 11, "other": 12}
)
...
change_dict(
    Q[3].column_dict, ["other", "none"], [{"insurance": 13, "other": 14, "none": 15}]
)
change_dict(
    Q[4].column_dict, ["other", "none"], [{"insurance": 13, "other": 14, "none": 15}]
)
...
for i in [2, 3, 4, 5]:
    Q[i].column_dict.update({"middlemen": 15, "other": 16})
...
for i in [0, 1, 3, 4]:
    Q[i].column_dict.update({"transportation": 11, "machinery": 12, "other": 13})
for i in [2, 5]:
    Q[i].column_dict.update({"other": 11})
...
Q[3].column_dict.update(
    {"marriage": 5, "healthcare": 6, "death": 7, "loss_of_income": 8}
)
Q[4].column_dict.update(
    {"marriage": 5, "healthcare": 6, "death": 7, "loss_of_income": 8}
)
...
Q = [
    Question_Encoder(
        ...
        answers={"yes": 1, "no": 2},
        column_dict={
            "could_relatives_help": 1,
            "household_skip_meal": 2,
            "house_unlit": 3,
            "too_sick_to_work": 4,
            "receive_support": 5,
        },
...
Q = [
    Question_Encoder(
        ...
        column_dict={
            "income_exceeds_outgoing?": 1,
            "fund_for_unplanned_expenses": 2,
            "pay_bills_ontime": 3,
            "savings_exceed_debts": 4,
        },
        country=country,
        survey="sr",
    )
...
Q = [
    Question_Encoder(
        ...
        column_dict={
            ...
            "short_term": 3,
            "live_for_today": 4,
            "future_determine_itself": 5,
            "work_hard": 6,
            "what_happens_happens": 7,
            "power_determines": 8,
        },
        country=country,
        survey="sr",
...
Q[1].column_dict.update(
    {"VLSA": 1, "ROSCA": 2, "money_guard": 3, "savings_collector": 4, "digital_card": 5}
)
Q[2].column_dict.update(
    {
        "VLSA": 1,
        "money_guard": 2,
        "savings_collector": 3,
...
data_dict["moz_sr"]["F50"].replace(
    {"1": 1, "2": 4, "3": 5, "5": 7, "6": 8, "Other (specify)": np.nan}, inplace=True
)
...
data_dict["uga_sr"]["F50"].replace(
    {1: 1, 2: 2, 3: 4, 4: 3, 5: 4, 6: 5, 7: 6, 8: 7, 9: 8}, inplace=True
)
...
Q = [
    Question_Encoder(
        ...
        column_dict={
            ...
            "lend_members": 3,
            "buy_for_members": 4,
            "guarantor_security": 5,
            "invest": 6,
            "purchase_tools": 7,
            "purchase_fixed_assets": 8,
            "funeral_emergency": 9,
            "help_save": 10,
        },
...
labels = ["F54", "F55", "F56"]
answers = [
    {"very important": 1, "somewhat_important": 2, "not_important": 3},
    {"yes": 1, "no": 2},
    {"yes": 1, "no": 2},
]
texts = [
    "For your agricultural activities, how important to you is it to borrow from the following?",
...
for c in ["bgd", "nga", "tan", "uga"]:
    count_instruments(
        c, ["F60_1", "F60_2", "F60_3", "F60_4", "F60_5", "F60_7"], "FORMAL_LOANS"
    )

count_instruments(
    "cdi",
    ["F60_1", "F60_2", "F60_3", "F60_4", "F60_5", "F60_7", "F60_16"],
    "FORMAL_LOANS",
)
...
Q = [
    Question_Encoder(
        ...
        label="D20",
        answers={
            "not_enough_for_food": 1,
            "food_and_clothes_only": 2,
            "can_save_a_bit": 3,
            "can_buy_some_expensive_goods": 4,
        },
        country=country,
        survey="hh",
    )
...
Q = [
    Question_Encoder(
        ...
        column_dict={
            ...
            "purchase_inputs": 2,
            "harvest_time": 3,
            "amount_crops_to_sell": 4,
            "when_where_to_sell": 5,
            "where_to_borrow": 6,
            "amount_livestock_to_sell": 7,
            "what_to_plant": 8,
        },
        country=country,
        survey="hh",
