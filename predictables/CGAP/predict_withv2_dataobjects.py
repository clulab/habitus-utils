...
NO_OF_BEST_FEATURES_TO_PRINT = (
    20  # even if the best combination has n features print only top 20
)
...
REGEX_QNS_TO_AVOID = [
    "F+"
]  # eg:'F+', '' if you dont know the exact qn name, but want to instead avoid training on a subset. eg. all finance related qns,. i.e starting with F
...
if USE_ALL_DATA == True:
    df_combined = CGAP.get_all_columns_given_country(
        QNS_TO_AVOID, COUNTRY, REGEX_QNS_TO_AVOID, SURVEY_QN_TO_PREDICT
    )
else:
...
if not MULTI_LABEL == True:
    maj_class, baseline = find_majority_baseline_binary(
        df_combined, SURVEY_QN_TO_PREDICT
    )
    logger.info(
        f"majority baseline={baseline}, majority class={maj_class} for all_data"
    )
...
if not MULTI_LABEL == True:
    ...
    logger.info(
        f"majority baseline={baseline}, majority class={maj_class} all data without nan"
    )
...
def find_best_feature_count():
    best_feature_indices, selectK = select_kbest_feature_indices(
        MAX_BEST_FEATURE_COUNT, x_train, y_train
    )
...
def do_training_predict_given_train_dev_splits_with_feature_selection(
    ...
    if not MULTI_LABEL == True:
        maj_class_dev, baseline_dev = find_majority_baseline_binary(
            dev, SURVEY_QN_TO_PREDICT
        )
        maj_class_train, baseline_train = find_majority_baseline_binary(
            train, SURVEY_QN_TO_PREDICT
        )
        logger.info(
            f"majority baseline in dev={baseline_dev}, majority class={maj_class_dev}"
        )
        logger.info(
            f"majority baseline in train={baseline_train}, majority class={maj_class_train}"
        )
    ...
    if DO_FEATURE_SELECTION == True:
        ...
        if MAX_BEST_FEATURE_COUNT < NO_OF_BEST_FEATURES_TO_PRINT:
            topn = get_topn_best_feature_names(
                selecK_best, x_train, MAX_BEST_FEATURE_COUNT
            )
        else:
            topn = get_topn_best_feature_names(
                selecK_best, x_train, NO_OF_BEST_FEATURES_TO_PRINT
            )
    else:
        ...
        y_dev_gold_selected = np.asarray(y_dev)
        if MULTI_LABEL == True:
            model = MultiOutputClassifier(model).fit(
                x_train_selected, y_train_gold_selected
            )
            ...
            for index, each_pred_column in enumerate(y_dev_pred.T):
                ...
                (
                    maj_class,
                    maj_class_baseline,
                ) = find_majority_baseline_binary_given_binary_column(
                ...
                multilabelFeature_accuracy[column_name] = (
                    acc,
                    maj_class_baseline,
                    maj_class,
                )
                ...
                logger.debug(
                    f"Majority class={maj_class}; Majority baseline={maj_class_baseline}"
                )
                logger.debug(
                    classification_report(
                        y_dev_gold_selected.T[index], each_pred_column
                    )
                )
                logger.debug("\n")
                logger.debug("****Confusion Matrix***")
    ...
    if DO_FEATURE_SELECTION == True:
        ...
        if MULTI_LABEL == True:
            ...
            logger.info(
                f"{header1_formatted}\t{header2_formatted}\t{header3_formatted}\t{header4_formatted}\t"
            )
            for k, v in multilabelFeature_accuracy.items():
                ...
                logger.info(
                    f"{k_formatted}\t{v0_formatted}\t{v1_formatted}\t{v2_formatted}"
                )
                all_accuracies.append(v)
            # logger.debug(f"average of all columns={np.mean(all_accuracies)}")


def do_training_predict_given_train_dev_splits_without_feature_selection(
    model, train, dev, test, best_feature_indices
):
    # separate out the gold/qn to predict so that we train only on the rest
    if MULTI_LABEL == True:
        y_train = train.filter(regex=(SURVEY_QN_TO_PREDICT + "_*"))
    ...
    if MULTI_LABEL == True:
        ...
        for index, each_pred_column in enumerate(y_dev_pred.T):
            ...
            logger.debug(
                classification_report(y_dev_gold_selected.T[index], each_pred_column)
            )
            logger.debug("\n")
            logger.debug("****Confusion Matrix***")
...
if DO_NFCV == True:
    ...
    selectksplit_datapoint_count = int(
        df_combined.shape[0] * NFCV_SELECTKBEST_SPLIT_PERCENTAGE / 100
    )
    ...
    best_feature_indices, selectK = select_kbest_feature_indices(
        MAX_BEST_FEATURE_COUNT, x_data_for_selectkbest, y_data_for_selectkbest
    )
    ...
    for index, (train_index, test_index) in enumerate(
        kf.split(data_not_used_for_selectkbest)
    ):
        ...
        dev = data_not_used_for_selectkbest.iloc[
            test_index
        ]  # in nfcv world there is only train test. but here using the word dev for maintaining consistency with non nfcv world
        ...
        accuracy = do_training_predict_given_train_dev_splits_without_feature_selection(
            model, train, dev, test, best_feature_indices
        )
        logger.info(f"nbest_feature_accuracy: {str(round(accuracy*100,2))}")

    maj_class, baseline = find_majority_baseline_binary(
        merge_n_test_folds, SURVEY_QN_TO_PREDICT
    )
    logger.info(
        f" for all n folds combined majority baseline={round(baseline,2)}, majority class={maj_class}:"
    )
    ...
    do_training_predict_given_train_dev_splits_with_feature_selection(
        model, train, dev, test
    )
