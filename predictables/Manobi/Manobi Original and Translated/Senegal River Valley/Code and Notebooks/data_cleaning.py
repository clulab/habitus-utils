...
def make_child_ht(child):
    return {
        child["X.ID"]: {"parent_id": child["Parent.ID"], "kids": [], "has_parent": 0}
        for child in [child.iloc[i] for i in range(len(child))]
    }


farmer_ht, plot_ht, season_ht = (
    make_child_ht(farmer),
    make_child_ht(plot),
    make_child_ht(season),
)
...
def check_parent_child(child_ht, parent_ht, child_name, parent_name):
    ...
    print(
        f"{kids_with_parents} of {len(child_ht)} {child_name}s exist in {parent_name} data"
    )
    parents_without_kids = np.sum([v["kids"] == [] for v in parent_ht.values()])
    print(
        f"{parents_without_kids} of {len(parent_ht)} {parent_name}s have no {child_name} data"
    )
...
print(
    f"We can trace a season record through plot, farmer and village for {sum([complete_chain(season_id) for season_id in season_ht.keys()])} records"
)
...
village.drop(
    columns=["fertilizerType_season2", "Parent.ID"], inplace=True, errors="ignore"
)
...
def correct_range(df, col, minval, maxval, report=False):
    ...
    if report:
        print(
            f"Before cleaning, {col} count = {df[col].count()}, min = {df[col].min()}, max = {df[col].max()}"
        )
    df[col].where((df[col] >= minval) & (df[col] <= maxval), inplace=True)
    if report:
        print(f"{df[col].describe()}")
    if report:
        print(f"Newly Nan records: {n - df[col].count()}\n")
...
correct_range(farmer, "loan_rate", 0.1, 20)

farmer["valid_children"] = farmer.num_children >= farmer.num_children_in_school
farmer["fraction_in_school"] = [
    s / n if (v and n > 0) else np.nan
    for s, n, v in zip(
        farmer.num_children_in_school, farmer.num_children, farmer.valid_children
    )
]
...
farmer.access_drinking_water.replace(
    to_replace={"RiviEre/Marigot": 0, "Puit": 1, "Pompe hydraulique": 1, "Robinet": 2},
    inplace=True,
)

farmer.access_healthcare.replace(
    to_replace={"not at all": 0, "bad": 0, "medium": 1, "good": 2, "very good": 2},
    inplace=True,
)
...
farmer.gender.replace(to_replace={"Mr": 0, "Mme": 1, "Mlle": 2}, inplace=True)
...
farmer.credit_from_coop.replace(
    to_replace={"never": 0, "sometimes": 1, "always": 2}, inplace=True
)
...
farmer.pledged_output.replace(to_replace={"no": 0, "partly": 1, "yes": 2}, inplace=True)

farmer.wants_bridge_loan.replace(to_replace={"no": 0, "yes": 1}, inplace=True)

farmer.interest_insurance.replace(
    to_replace={"no": 0, "maybe": 1, "Peut-Itre": 1, "yes": 2}, inplace=True
)
...
farmer.use_ag_services.replace(
    to_replace={"Aucun": 0, "ANCAR": 2, "Autres": 1}, inplace=True
)
...
def map_to_bins(attribute, bins):
    farmer[attribute + "_binned"] = pd.cut(
        farmer[attribute], bins, labels=list(range(len(bins) - 1)), right=False
    ).astype(float)
...
map_to_bins(
    "num_wives", [0, 1, 2, farmer.num_wives.max() + 1]
)  # 0 for 0 wives, 1 for 1 wife, 2 for >1 wife
...
def education(row):
    ...
    if row.education_level == "AlphabEtisE" or row.education_level == "Non AlphabEtisE":
        ...
        row.education_level == "Primaire"
        or row.education_level == "SupErieur"
        or row.education_level == "Secondaire"
    ):
        return 1
    else:
        return np.nan
...
def women_role(row):
    if row.num_wives > 0:
        if row.wife_trades == 0 and row.wife_food_crops == 0:
            return 0
        if (row.wife_trades == 1 or row.wife_food_crops == 1) and (
            not (row.wife_trades == 1 and row.wife_food_crops == 1)
        ):
            return 1
...
farmer["plot_list"] = farmer.apply(
    lambda x: parent_to_plot_ht.get(x.id) or np.nan, axis=1
)
...
farmer_attributes = [
    ...
    "use_ag_services",
    "ag_services_quality",
    "ag_services_frequency",
    "interest_ag_services",
    "last_price",
    "num_plots",
    "total_ha",
    "fraction_in_school",
    "fraction_in_school_binned",
    "num_older_children_binned",
    "num_young_children_binned",
    "income",
    "nonzero_income",
    "income_binned",
    "num_plots_binned",
    "years_farming_binned",
    "age_binned",
    "num_children_binned",
    "num_wives_binned",
...
village.index = (
    village.village_id
)  # to ensure that the following concat aligns by village
...
id_and_name = (
    farmer[["coop_id", "coop_name"]].groupby("coop_id", as_index=True).agg(coop_name)
)
