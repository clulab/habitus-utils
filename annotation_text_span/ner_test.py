from annotation_ner_span import NlpSpan
...
def main():
    ...
    test_sent1 = (
        "Silly Paul Anna Geogre killed his plants by applying too much fertilizer."
    )
    test_output1 = processor.get_ner_span(test_sent1)
    ...
    test_output2 = processor.get_ner_span(test_sent2)
    ...
    test_output3 = processor.get_nltk_ner_span(test_sent3)
    ...
    test_output4 = processor.get_ner_span(test_sent4)
    ...
    test_output5 = processor.get_ner_span(test_sent5)
    ...
    test_output6 = processor.get_ner_span(test_sent6)
    ...
    test_sent7 = "Autonomous cars shift insurance liability towards manufactures and Autonomous cars"
    test_output7 = processor.get_syntactic_span(test_sent7)
    assert len(test_output7["pos"]) == 10
    assert len(test_output7["index"]) == 7
    assert test_output7["index"][0] == [0, 2]
    assert test_output7["index"][2] == [3, 5]
    assert test_output7["index"][4] == [6, 7]
    assert test_output7["pos"][0] == "ADJ"
    assert test_output7["pos"][9] == "NOUN"

    test_sent8 = "Silly Paul killed tomato and rice plants in Atlanta, Georgia."
    test_output8 = processor.get_syntactic_span(test_sent8)
    assert len(test_output8["pos"]) == 12
    assert len(test_output8["index"]) == 10
    assert test_output8["index"][0] == [0, 2]
    assert test_output8["index"][3] == [4, 5]
    assert test_output8["index"][4] == [5, 7]
    assert test_output8["pos"][2] == "VERB"
    print(
        "*******************************SPACY tests PASSED*******************************\n"
    )
