...
parser.add_argument(
    "--num_sents",
    help="Number of sentences to check if target countries and at least one candidate countries occur together",
    type=int,
    default=1,
    nargs="?",
    const=3,
...
parser.add_argument(
    "--target_country",
    help="Country with which we want to find similar countries.",
    default="bangladesh",
)
parser.add_argument(
    "--similar_countries",
    help="List of countries that we want to calculate similarity score.",
    default=["mozambique", "nigeria", "uganda"],
    type=list,
)
parser.add_argument(
    "--pdf_storage",
    help="Location on the device to store pdfs downloaded based on specified credentials.",
    default="dowloaded_pdfs",
)
parser.add_argument(
    "--factors_of_interest",
    help="list of factor that we want to measure country similarities e.g., education level, income",
    default=["education level", "laws"],
    type=list,
)
parser.add_argument(
    "--google_developer_key",
    help="The Google API developer key which can be created as instructed in README.md",
    default="AIzaSyCAzULm7A5v3-702_TQ1xwF66J2mCa2xxA",
)
parser.add_argument(
    "--custom_search_id",
    help="The Google custom search ID, which can be created as instructed in README.md",
    default="b47fb500623cddc82",
)
parser.add_argument(
    "--overwrite",
    help="Overwrite the pdf download folder",
    nargs="?",
    const=True,
    default=False,
    type=bool,
...
def get_pdf_links_related_to_target_and_similar_countries(
    target_country, similar_countries, factors
):
    pdflinks = []
    ...
    for i in range(91, 200, 10):
        for country in similar_countries:
            for factor in factors:
                ...
                partial_links, isEmpty = google_pdf_links_crawler(
                    args.google_developer_key,
                    args.custom_search_id,
                    query,
                    start=i,
                    lr="lang_en",
                    fileType="pdf",
...
def main():
    pdf_links = get_pdf_links_related_to_target_and_similar_countries(
    ...
    if not os.path.exists(pdf_storage):
        ...
        if args.overwrite:
            ...
            sys.exit(
                1
            )  # give 1 here so that the linux system know that we encounter an error.
    ...
    pdf_to_txt(
        pdf_storage,
        os.path.join(args.pdf_storage, args.target_country, "plain_text"),
        args.overwrite,
    )
    print("Converted pdf files to txt!")
    # calculating similarity scores
    similarity_scores = get_country_similarity_scores(
        os.path.join(args.pdf_storage, args.target_country, "plain_text"),
        args.num_sents,
        args.target_country,
        args.similar_countries,
        args.factors_of_interest,
    )
