...
def download_pdfs_from_links(pdflinks, download_folder, target_country):
	...
	for i in range(len(pdflinks)):
		if pdflinks[i] not in already_downloaded:
			...
			success = 1
			try:
				download.urlretrieve(pdflinks[i], os.path.join(download_folder, f'{target_country}_{i}.pdf'))
