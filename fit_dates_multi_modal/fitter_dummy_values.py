...
if GENERATE_DUMMY_DATA:
    distribution1 = numpy.random.normal(
        loc=(time_range[0] + time_range[1] / 2) * 0.5,
        scale=(time_range[1] - time_range[0]) * _EMPIRICAL_SCALE_RATIO,
        size=_DISTRIBUTION_SIZE,
    )
    distribution2 = numpy.random.normal(
        loc=(time_range[1] / 2 + time_range[1]) * 0.5,
        scale=(time_range[1] - time_range[0]) * _EMPIRICAL_SCALE_RATIO,
        size=_DISTRIBUTION_SIZE,
    )
...
axes.set(
    ...
    xlabel="Dates which are edges of each bin",
