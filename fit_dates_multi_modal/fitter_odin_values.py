...
INPUTS = [
    "XXXX-07-07 -- XXXX-07-22",
    "XXXX-07-19 -- XXXX-08-04",
    "XXXX-07-19 -- XXXX-08-04",
    "XXXX-03-03 -- XXXX-03-11",
    "XXXX-07-14 -- XXXX-07-31",
    "2005-03-18",
]
...
def check_if_just_year(input):
    ...
    # 2. Dates that include only year information should be removed.
    if (len(split_by_dash) > 1 and split_by_dash[1] == "XX") and split_by_dash[
        2
    ] == "XX":
        return True
    ...
    if is_range:
        ...
        if flag1 and flag2:
            start_date_as_datetime = datetime.strptime(
                start_date_with_custom_year, "%Y-%m-%d"
            )
            end_date_as_datetime = datetime.strptime(
                end_date_with_custom_year, "%Y-%m-%d"
            )
            days_count = get_days_count_in_range(
                start_date_as_datetime, end_date_as_datetime
            )
            dates_in_range = list_all_days_in_a_range(
                start_date_as_datetime, end_date_as_datetime, days_count
            )
...
def reverse_dates(date, pos=None):
    indiv = time.strftime(
        _DATE_FORMAT_ONLY_MMDD, time.localtime(date * _SCALE_RATIO_FOR_DRAWING)
    )
    return indiv
...
axes.set(
    xlim=[bins[0], bins[len(bins) - 1]],
    xlabel="sowing dates",
    ylabel="No of dates per bin ",
    title=title_custom,
)
...
for bndw in all_bdw:
    kde = KernelDensity(kernel="gaussian", bandwidth=bndw).fit(
        X=distribution.reshape(-1, 1)
    )
