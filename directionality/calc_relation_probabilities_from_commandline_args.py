from calc_relation_probabilities_from_disk_inputs import (
    calc_average_probabilities_across_synonyms,
)
import argparse
...
class DirectionalProbabilities:
    ...
    def get_prob_across_models(self, threshold=0, check_directionality=False):
        ...
        for each_model in self.models:
            ...
            avg_prob_rev = calc_average_probabilities_across_synonyms(
                self.effects,
                self.causes,
                self.triggers,
                model,
...
def parse_arguments():
    ...
    argparser.add_argument(
        "--causes",
        nargs="+",
        help="list of cause variables and their synonyms as strings separated by space e.g., education education_standard",
    )
    argparser.add_argument(
        "--effects",
        nargs="+",
        help="list of effect variables and their synonyms as strings separated by space e.g., income income_level",
    )
    argparser.add_argument(
        "--triggers",
        nargs="+",
        help="list of trigger_verbs as strings separated by space e.g., improves accelerates boosts",
    )
    argparser.add_argument(
        "--models",
        nargs="+",
