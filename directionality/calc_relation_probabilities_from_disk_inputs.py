...
def parse_arguments():
    ...
    argparser.add_argument(
        "input_file_name",
        help="name of the input file where causal and effect variables are kept",
    )
...
def calc_rel_prob(
    cause, effect, triggers, tokenizer, model, threshold=0, check_directionality=False
):
    """
        ...
        for i in range(len(effect_tokens)):
            ...
            if check_directionality == True:
                if prob_effect > threshold:
                    print(
                        f"found that there is a relation.  {cause} {trigger}  {effect} with probability of {prob_effect} which is > given threshold:{threshold}"
                    )
                    exit()
...
def calc_average_probabilities_across_models_write_disk(
    overall_prob_averages_across_models, output_file_overall
):
...
def calc_average_probabilities_across_synonyms(
    cause_synonyms,
    effect_synonyms,
    triggers,
    model,
        ...
        for id_cause, cause_synonyms in all_causes:
            for id_effect, effect_synonyms in all_effects:
                if not (id_cause == id_effect):
                    ...
                    avg_prob = calc_average_probabilities_across_synonyms(
                        cause_synonyms, effect_synonyms, triggers, model, tokenizer
                    )
                    output = (
                        f"{id_cause}\t{id_effect}\t{trigger_group_name}\t{avg_prob}\n"
                    )
                    ...
                    unique_id_datapoint = (
                        f"{id_cause}\t{id_effect}\t{trigger_group_name}"
                    )
                    fill_dict(
                        overall_prob_averages_across_models,
                        unique_id_datapoint,
...
if __name__ == "__main__":
    ...
    for each_model in MLM_MODELS:
        ...
        calc_average_probabilities(
            all_causes=data.items(),
            all_triggers=promote_inhibit_triggers.items(),
            all_effects=data.items(),
            model=model,
            tokenizer=tokenizer,
            overall_prob_averages_across_models=overall_prob_averages_across_models,
        )
    ...
    calc_average_probabilities_across_models_write_disk(
        overall_prob_averages_across_models, output_file_overall
    )
