...
from calc_relation_probabilities_from_commandline_args import (
...
def parse_arguments():
    ...
    argparser.add_argument(
        "--cause", nargs="+", help="cause variables  e.g., education ",
    )
    argparser.add_argument("--effect", nargs="+", help="effect variable e.g., income ")
    argparser.add_argument(
        "--models",
        nargs="+",
...
if __name__ == "__main__":
    ...
    if args.use_polarity == True:
        ...
        average_calculator = DirectionalProbabilities(
            causes=args.cause,
            effects=args.effect,
            models=args.models,
            triggers=set(all_verbs),
        )
        dir1, dir2 = average_calculator.get_prob_across_models(
            threshold=args.threshold, check_directionality=True
        )
        if (dir1 > args.threshold) or (dir2 > args.threshold):
            print(
                f"found that some relation between {args.cause} to {args.effect} exists"
            )
            exit()
        else:
            print(
                f"No relation exists between {args.cause} to {args.effect} for the given threshold"
            )
